(function(){
  ...
  function normalize(raw,meta={}){
    ...
    data.sessions=data.sessions.filter(x=>x.recordId||x.examName).filter(x=>isEnabled(x.enabled));ensureSessionRecordIds(data.sessions);applyExamPeriods(data.sessions);
