(function(){
  ...
  function topikNoteBlock(notes,label){const m=String(notes||'').match(new RegExp(`(?:^|\\n)\\s*${label}\\s*[:：]?\\s*\\n?([\\s\\S]+)$`));return m?m[1].trim():''}
  function topikScheduleTimes(schedule){const matches=[...String(schedule||'').matchAll(/(\d{1,2})[月.](\d{1,2})日?\.?\s*(?:[（(][^）)\n]*[）)])?\s*(\d{1,2}:\d{2})/g)];const format=m=>m?`${Number(m[1])}月${Number(m[2])}日 ${m[3]}`:'';return{start:format(matches[0]),deadline:format(matches[1])}}
  ...
  function applyTopikNotes(sessions){sessions.forEach(s=>{if(!/TOPIK/i.test(String(s.examId||s.examName||''))||!/韩国/.test(`${s.region||''}${s.sessionName||''}${s.notes||''}`))return;const exam=DateUtils.parseExcelDate(s.examDate),year=Number(s.year||(exam&&exam.getFullYear())),notes=s.notes;s.noteRegistrationSchedule=topikNoteBlock(notes,'报名日程');const scheduleTimes=topikScheduleTimes(s.noteRegistrationSchedule);s.noteRegistrationTime=topikNoteValue(notes,'报名时间')||scheduleTimes.start;s.noteRegistrationDeadline=topikNoteValue(notes,'报名截止(?:时间)?')||scheduleTimes.deadline;s.noteTicketPrint=topikNoteValue(notes,'准考证打印时间');s.noteScoreTime=topikNoteValue(notes,'出分时间');if(!s.ticketPrintStart&&s.noteTicketPrint)s.ticketPrintStart=monthDayDate(s.noteTicketPrint,year)})}
  function applyJlptNotes(sessions){sessions.forEach(s=>{if(!/JLPT|日本语能力/i.test(String(s.examId||s.examName||'')))return;const exam=DateUtils.parseExcelDate(s.examDate),year=Number(s.year||(exam&&exam.getFullYear())),m=String(s.notes||'').match(/准考证打印(?:时间)?\s*[:：]?\s*([^,，。;；\n]+)/);s.noteTicketPrint=m?m[1].trim():'';if(!s.ticketPrintStart&&s.noteTicketPrint)s.ticketPrintStart=monthDayDate(s.noteTicketPrint,year)})}
  ...
  function normalize(raw,meta={}){
    ...
    data.sessions=data.sessions.filter(x=>x.recordId||x.examName).filter(x=>isEnabled(x.enabled));ensureSessionRecordIds(data.sessions);applyExamPeriods(data.sessions);applyTopikNotes(data.sessions);applyJlptNotes(data.sessions);
