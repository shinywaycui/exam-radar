(function(){
  ...
  function dynamicMomentsBody(type,event,vars,today){
    const exam=vars['考试名称'],session=vars['考试场次'],level=vars['考试级别'],examDate=vars['考试日期'],scoreDate=vars['预计出分日期'],date=shortDate(event.eventDate),seed=seedFor(event,today,type),tomorrow=event.daysUntil===1,when=event.daysUntil===0?'今天':tomorrow?`明天（${date}）`:event.daysUntil>0?`${date}，还有${event.daysUntil}天`:`已过去${Math.abs(event.daysUntil)}天`,cta=pick(['想了解备考节奏和课程安排，欢迎随时滴滴～','需要考试规划的宝子，可以来聊聊你的目标～','还没理清时间线的同学，欢迎来找我一起规划～','想稳稳拿下目标等级，现在就可以开始准备啦～'],seed,7);
