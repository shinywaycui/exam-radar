(function(){
  ...
  function curatedMomentCopy(type,event,vars,today){let pool;if(event.eventType==='正式报名截止'){if(event.daysUntil<0){const voice=examVoices[voiceKey(event)]||examVoices.generic,name=eventName(event),deadline=vars['报名截止日期'];return{title:`${voice.flag}${name}报名已截止`,body:`⏰报名已于${deadline||vars['节点日期']}截止\n✅已报名：及时进入系统备考并保存报名凭证\n📌未报名：关注下一场考试安排，提前规划学习与报名时间`}}pool=registrationCopyLibrary.deadline}else if(event.eventType==='报名注册开始'||event.eventType==='正式报名开始'){if(event.daysUntil<0){const voice=examVoices[voiceKey(event)]||examVoices.generic,name=eventName(event),deadline=vars['报名截止日期'];return{title:`${voice.flag}${name}报名已开启！`,body:`📣报名通道已经开放${deadline?`\n⏰截止时间：${deadline}`:''}\n👇请尽早确认报考级别、考点、个人信息及缴费状态\n完成报名后，就可以安心进入备考节奏啦！`}}pool=registrationCopyLibrary[event.daysUntil===0?'today':'upcoming']}else if(event.eventType==='出分日期')pool=scoreCopyLibrary[event.daysUntil===0?'today':'upcoming'];else return null;const voice=examVoices[voiceKey(event)]||examVoices.generic,name=eventName(event),values={...vars,'考试名称':name};let text=pick(pool,seedFor(event,today,type));text=text.replace('🇯🇵/🇰🇷/🇩🇪',voice.flag).replace(/【([^】]+)】/g,(_,key)=>values[key]!=null&&values[key]!==''?String(values[key]):'以官方通知为准');const lines=text.split('\n');return{title:lines.shift(),body:lines.join('\n')}}
