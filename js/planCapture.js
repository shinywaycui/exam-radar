(function(){
  ...
  async function create(){
    ...
    const rect=source.getBoundingClientRect(),width=Math.ceil(Math.max(source.scrollWidth,rect.width)),height=Math.ceil(Math.max(source.scrollHeight,rect.height));
    const copy=source.cloneNode(true);inlineStyles(source,copy);copy.style.width=`${width}px`;copy.style.height=`${height}px`;copy.style.margin='0';copy.style.transform='none';copy.setAttribute('xmlns','http://www.w3.org/1999/xhtml');
  ...
  function syncTrigger(){if(trigger)trigger.hidden=!app?.querySelector('.matrix-page')}
