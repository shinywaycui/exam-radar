(function(){
  ...
  function getTodayRadar(data,today=new Date()){
    ...
    const upcomingEvents=events.filter(e=>e.daysUntil>0&&e.daysUntil<=7).map(enrich).sort(sortByTiming),recentEvents=events.filter(e=>e.daysUntil>=-1&&e.daysUntil<=7).map(enrich).sort(sortByTiming),extendedEvents=events.filter(e=>e.daysUntil>7&&e.daysUntil<=31).map(enrich).sort(sortByTiming),momentPool=[...recentEvents,...extendedEvents].filter((e,i,a)=>a.findIndex(x=>x.eventId===e.eventId)===i),momentsRecommendations=momentPool.filter(e=>!e.rule||DataNormalizer.isEnabled(e.rule.generateMoments));
