(function(){
  ...
  function getTodayRadar(data,today=new Date()){
    ...
    const upcomingEvents=events.filter(e=>e.daysUntil>0&&e.daysUntil<=7).map(enrich).sort(sortByTiming),momentPool=events.filter(e=>e.daysUntil>=-7&&e.daysUntil<=7).map(enrich).sort(sortByTiming),momentsRecommendations=momentPool.filter(e=>!e.rule||DataNormalizer.isEnabled(e.rule.generateMoments));
