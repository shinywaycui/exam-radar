(function(){
  ...
  function getTodayRadar(data,today=new Date()){
    ...
    const month=DateUtils.parseExcelDate(today),monthEvents=events.filter(e=>e.year===month.getFullYear()&&e.month===month.getMonth()+1&&e.daysUntil>=-7).map(enrich),nearby=events.filter(e=>e.daysUntil>=0&&e.daysUntil<=31).map(enrich),monthlyFocus=[...monthEvents,...nearby].filter((e,i,a)=>a.findIndex(x=>x.eventId===e.eventId)===i).sort(sortByTiming);
    const upcomingEvents=events.filter(e=>e.daysUntil>0&&e.daysUntil<=7).map(enrich).sort(sortByTiming),recentEvents=events.filter(e=>e.daysUntil>=-1&&e.daysUntil<=7).map(enrich).sort(sortByTiming),extendedEvents=events.filter(e=>e.daysUntil>7&&e.daysUntil<=31).map(enrich).sort(sortByTiming),momentPool=[...recentEvents,...extendedEvents].filter((e,i,a)=>a.findIndex(x=>x.eventId===e.eventId)===i),momentsRecommendations=momentPool.filter(e=>!e.rule||DataNormalizer.isEnabled(e.rule.generateMoments)).slice(0,3);
    const updateActions=todayUpdates.slice(0,3).map((e,i)=>({...e,advisorActions:`核对${e.examName}最新信息；同步相关顾问与学员`,priority:110-i})),actionSource=[...todayEvents.map(enrich),...upcomingEvents,...updateActions].slice(0,10),highPriority=events.filter(e=>e.rule&&Math.abs(e.daysUntil)<=31&&DataNormalizer.isEnabled(e.rule.generateWechat)).map(enrich).sort(sortRadar).slice(0,8);
